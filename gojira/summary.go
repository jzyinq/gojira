...
func (s *Summary) update() {
	...
	workingHours := workingHoursInMonthToPresentDay(app.time.Year(), app.time.Month(), app.holidays)
	difference := workingHoursAbsoluteDiff(workingHours, app.workLogs.TotalTimeSpentToPresentDay())
...
func workingHoursInMonthToPresentDay(year int, month time.Month, holidays *Holidays) int {
...
func workingHoursAbsoluteDiff(workingHours int, totalTimeSpent int) int {
	difference := workingHours*60*60 - totalTimeSpent
	if difference < 0 {
